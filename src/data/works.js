export default [
  {
    title: 'Musium',
    description: 'A Spotify inspired music streaming site that uses the Shazam Core API. Get lyric information of a song, show related songs, artists, related artists, top charts global and based off region, and more. IMPORTANT: If you have an adblocker, the `Around You` section will not allow for API requests.',
    imageUrl: 'https://i.postimg.cc/bY9zQ3kn/musium.png',
    tech: ['HTML', 'Tailwind CSS', 'React JS', 'Vite', 'Redux', 'Axios', ],
    workUrl: 'https://musium-37a14b.netlify.app/',
    githubUrl: 'https://github.com/MCabal77/musium-app',
  },
  {
    title: 'Banking App React Visualization',
    description: 'First modern design implementation on a website. This is for proof of concept and currently is not set up with any databases or APIs. ',
    imageUrl: 'https://i.postimg.cc/1XmfBm5g/Bank-React1.png',
    tech: ['HTML', 'Tailwind CSS', 'React JS', 'Vite', ],
    workUrl: 'https://polite-wisp-a0b2ed.netlify.app/',
    githubUrl: 'https://github.com/MCabal77/modern-landing-page-visualization',
  },
  {
    title: 'Landing Page Visualization',
    description: 'A landing page built solely on HTML5 and CSS3. Currently does not support mobile responsiveness.',
    imageUrl: 'https://i.postimg.cc/y631q32V/webpage-1.png',
    tech: ['HTML', 'CSS'],
    workUrl: 'https://mcabal77.github.io/landing-page-visualization/',
    githubUrl: 'https://github.com/MCabal77/landing-page-visualization',
  },
  
]